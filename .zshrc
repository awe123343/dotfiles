...
# Homebrew (must be before zinit for plugins to find commands)
...
if [ ! -d $ZINIT_HOME/.git ]; then
  git clone --depth 1 git@github.com:zdharma-continuum/zinit.git "$ZINIT_HOME" || \
  git clone --depth 1 https://github.com/zdharma-continuum/zinit.git "$ZINIT_HOME"
fi
source "${ZINIT_HOME}/zinit.zsh"

# Load Powerlevel10k (Instant)
zinit ice depth=1; zinit light romkatv/powerlevel10k

# Load zsh-defer (Instant)
zinit light romkatv/zsh-defer

# Load autosuggestions immediately (avoid delay)
...
# OMZ git library (required for git_current_branch, ggfl, etc.)
...
# dircolors-solarized (load immediately, turbo causes issues with gdircolors)
